    ...
    <div class="col-md-8">
        <div class="card">
            <div class="card-header">
                <h5 class="title">{{ __('Add Souvenir') }}</h5>
            </div>
            <form method="post" action="{{ route('souvenirs.add_souvenir') }}" enctype="multipart/form-data">
                <div class="card-body">
                    @csrf
                    ...
                    <div class="form-group{{ $errors->has('souvenir_date') ? ' has-danger' : '' }}">
                        ...
                        <input type="date" name="souvenir_date" class="form-control{{ $errors->has('souvenir_date') ? ' is-invalid' : '' }}" placeholder="{{ __('memory date') }}" required>
